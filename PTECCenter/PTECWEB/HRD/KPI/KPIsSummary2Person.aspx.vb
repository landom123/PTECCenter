...
Public Class KPIsSummary2Person
...
endprocess:
    ...
    Private Sub setDetail_Project(dt As DataTable)
        ...
        headerRow.Cells.Add(New TableCell() With {.Text = "Score (หัวหน้างาน)", .CssClass = "text-center align-middle font-weight-bold"})
        ...
        For Each row As DataRow In dt.Rows
            If row("object_type").ToString().ToLower() = "project" And row("UserType").ToString().ToLower() = "owner" Then
                ...
                Dim scoreSpan As New LiteralControl($"<span class='mx-sm-3 text-center align-middle text-success'>{HttpUtility.HtmlEncode(row("Approval_rate").ToString()).Replace(vbCrLf, "<br>").Replace("\n", "<br>")}</span>")
    ...
    Private Sub setDetail_TrainingApprover()
        Dim hasTraining As Boolean = False
        Dim htmlContent As New StringBuilder()
        ...
        txtTrainingApprover.Text = If(hasTraining, htmlContent.ToString(), "-")
    End Sub
    ...
    Private Sub setDetail_Career(dt_career As DataTable, dt_txtarea As DataTable)
        ...
        If dt_career.Rows.Count > 0 Then
            ...
            headerRow.Cells.Add(New TableCell() With {.Text = "Score (พนักงาน)", .CssClass = "text-center align-middle font-weight-bold mx-sm-3"})
            headerRow.Cells.Add(New TableCell() With {.Text = "Actual (พนักงาน)", .CssClass = "text-center align-middle font-weight-bold mx-sm-3 w-25"})
            headerRow.Cells.Add(New TableCell() With {.Text = "Score (หัวหน้างาน)", .CssClass = "text-center align-middle font-weight-bold mx-sm-3"})
            headerRow.Cells.Add(New TableCell() With {.Text = "Actual (หัวหน้างาน)", .CssClass = "text-center align-middle font-weight-bold mx-sm-3 w-25"})
            ...
            For Each row As DataRow In dt_career.Rows
                ...
                If row("object_type").ToString = "career" And row("UserType").ToString().ToLower = "owner" Then
                    ...
                    ' ค้นหาข้อมูลใน dt_career
                    Dim matchingCareerApproverRows = dt_career.Select($"label = '{row("label").ToString()}' AND usertype = 'approver'")
                    Dim reasonCareerApproverValue As String = If(matchingCareerApproverRows.Length > 0 AndAlso Not IsDBNull(matchingCareerApproverRows(0)("txtRate")), matchingCareerApproverRows(0)("txtRate").ToString.Replace(vbCrLf, "<br>").Replace(vbCr, "<br>").Replace(vbLf, "<br>").Replace("\n", "<br>"), "")

                    ' Score Approver
                    Dim scoreApproverCell As New TableCell()
                    scoreApproverCell.CssClass = "text-center align-middle"
                    Dim scoreApproverSpan As New LiteralControl($"<span class='mx-sm-3 text-success'>{reasonCareerApproverValue}</span>")
                    scoreApproverCell.Controls.Add(scoreApproverSpan)
                    tableRow.Cells.Add(scoreApproverCell)

                    ' ค้นหาข้อมูลใน dt_txtarea
                    Dim matchingApproverRows = dt_txtarea.Select($"parent_id = '{row("KPIformDtl_ID").ToString()}' AND usertype = 'approver'")
                    Dim reasonApproverValue As String = If(matchingApproverRows.Length > 0 AndAlso Not IsDBNull(matchingApproverRows(0)("txt_detail")), matchingApproverRows(0)("txt_detail").ToString.Replace(vbCrLf, "<br>").Replace(vbCr, "<br>").Replace(vbLf, "<br>").Replace("\n", "<br>"), "")

                    ' Reason
                    Dim reasonApproverCell As New TableCell()
                    reasonApproverCell.CssClass = "text-left align-middle"
                    Dim reasonApproverSpan As New LiteralControl($"<span class='mx-sm-3'>{reasonApproverValue}</span>")
                    reasonApproverCell.Controls.Add(reasonApproverSpan)
                    tableRow.Cells.Add(reasonApproverCell)
    ...
    Private Sub setDetail_KPIs(dt_kpi As DataTable, dt_ans_kpi As DataTable)
        ...
        If dt_kpi.Rows.Count > 0 Then
            ...
            For Each row As DataRow In dt_kpi.Rows
                ...
                ' ค้นหาข้อมูลใน dt_ans_kpi
                Dim matchingApproverRows = dt_ans_kpi.Select($"kpi_code = '{row("kpi_code").ToString()}' AND usertype = 'approver'")
                Dim scoreApproverValue As Decimal = If(matchingApproverRows.Length > 0 AndAlso Not IsDBNull(matchingApproverRows(0)("txtRate")), Convert.ToDecimal(matchingApproverRows(0)("txtRate")), 0.00)
                Dim actualApproverValue As String = If(matchingApproverRows.Length > 0 AndAlso Not IsDBNull(matchingApproverRows(0)("txtReason")), matchingApproverRows(0)("txtReason").ToString(), "")
                actualApproverValue = HttpUtility.HtmlEncode(actualApproverValue).Replace("\n", "<br>")

                Dim scoreApproverSpan As New LiteralControl($"<span class='mx-sm-3 text-success'>{scoreApproverValue}</span>")
                scoreApproverCell.Controls.Add(scoreApproverSpan)
                tableRow.Cells.Add(scoreApproverCell)

                ' Actual
                Dim actualApproverCell As New TableCell()
                actualApproverCell.CssClass = "text-left align-middle"
                Dim actualApproverSpan As New LiteralControl($"<span class=''>{actualApproverValue}</span>")
                actualApproverCell.Controls.Add(actualApproverSpan)
                tableRow.Cells.Add(actualApproverCell)
